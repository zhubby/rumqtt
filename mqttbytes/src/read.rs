...
pub fn read(stream: &mut BytesMut, protocol: Protocol, max_size: usize) -> Result<Packet, Error> {
    ...
    let packet = match packet_type {
        ...
        PacketType::PubRel => Packet::PubRel(PubRel::read(fixed_header, packet, protocol)?),
        PacketType::PubComp => Packet::PubComp(PubComp::read(fixed_header, packet, protocol)?),
        PacketType::Subscribe => {
            Packet::Subscribe(Subscribe::read(fixed_header, packet, protocol)?)
        }
        PacketType::SubAck => Packet::SubAck(SubAck::read(fixed_header, packet, protocol)?),
