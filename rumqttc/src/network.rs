...
pub struct Io {
    pub(crate) i: IoReader,
    pub(crate) o: IoWriter,
}

impl Io {
    pub fn new(socket: impl I + O + 'static, max_incoming_size: usize) -> Io {
        let (i, o) = tokio::io::split(socket);
        Io {
            i: IoReader::new(i, max_incoming_size),
            o: IoWriter::new(o),
        }
    }
}
...
/// advantage of pre-allocation, buffering when appropriate to
/// achieve performance
pub struct IoReader {
    /// Socket for IO
    socket: Box<dyn I>,
...
impl IoReader {
    pub fn new(socket: impl I + 'static, max_incoming_size: usize) -> IoReader {
        let socket = Box::new(socket) as Box<dyn I>;
        IoReader {
            socket,
            pending: 0,
...
impl IoWriter {
    pub fn new(socket: impl O + 'static) -> IoWriter {
        let socket = Box::new(socket) as Box<dyn O>;
        IoWriter {
            socket,
...
pub trait I: AsyncRead + Send + Sync + Unpin {}
impl<T> I for T where T: AsyncRead + Unpin + Send + Sync {}

pub trait O: AsyncWrite + Send + Sync + Unpin {}
impl<T> O for T where T: AsyncWrite + Unpin + Send + Sync {}
